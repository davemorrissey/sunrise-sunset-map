<li id="country-AF"><a class="country" onclick="toggleCountry('AF')"><img src="img/flag/AF.png" alt="Afghanistan"/>Afghanistan</a>
    <ol id="cities-AF">
        <li><a onclick="showCity('Herat', 34.34, 62.19)"><i class="fa fa-angle-right"></i> Herat</a></li>
        <li><a onclick="showCity('Kabul', 34.5166667, 69.1833333)"><i class="fa fa-angle-right"></i> Kabul</a></li>
        <li><a onclick="showCity('Kandahar', 31.61, 65.7)"><i class="fa fa-angle-right"></i> Kandahar</a></li>
        <li><a onclick="showCity('Maz\u0101r-e Shar\u012Bf', 36.7069444, 67.1122222)"><i class="fa fa-angle-right"></i> Maz&#257;r-e Shar&#299;f</a></li>
    </ol>
</li>
<li id="country-DZ"><a class="country" onclick="toggleCountry('DZ')"><img src="img/flag/DZ.png" alt="Algeria"/>Algeria</a>
    <ol id="cities-DZ">
        <li><a onclick="showCity('Algiers', 36.7630556, 3.0505556)"><i class="fa fa-angle-right"></i> Algiers</a></li>
        <li><a onclick="showCity('Bab Ezzouar', 36.7166667, 3.1833333)"><i class="fa fa-angle-right"></i> Bab Ezzouar</a></li>
        <li><a onclick="showCity('Batna', 35.5559710497557, 6.17414474487305)"><i class="fa fa-angle-right"></i> Batna</a></li>
        <li><a onclick="showCity('Constantine', 36.365, 6.6147222)"><i class="fa fa-angle-right"></i> Constantine</a></li>
        <li><a onclick="showCity('Oran', 35.6911111, -0.6416667)"><i class="fa fa-angle-right"></i> Oran</a></li>
    </ol>
</li>
<li id="country-AR"><a class="country" onclick="toggleCountry('AR')"><img src="img/flag/AR.png" alt="Argentina"/>Argentina</a>
    <ol id="cities-AR">
        <li><a onclick="showCity('Buenos Aires', -34.5761256318848, -58.4088134765625)"><i class="fa fa-angle-right"></i> Buenos Aires</a></li>
        <li><a onclick="showCity('C\u00F3rdoba', -31.4, -64.1833333)"><i class="fa fa-angle-right"></i> C&oacute;rdoba</a></li>
        <li><a onclick="showCity('La Plata', -34.9313889, -57.9488889)"><i class="fa fa-angle-right"></i> La Plata</a></li>
        <li><a onclick="showCity('Mar del Plata', -38.0, -57.55)"><i class="fa fa-angle-right"></i> Mar del Plata</a></li>
        <li><a onclick="showCity('Mendoza', -32.8833333, -68.8166667)"><i class="fa fa-angle-right"></i> Mendoza</a></li>
        <li><a onclick="showCity('Quilmes', -34.7202778, -58.2694444)"><i class="fa fa-angle-right"></i> Quilmes</a></li>
        <li><a onclick="showCity('Rosario', -32.9511111, -60.6663889)"><i class="fa fa-angle-right"></i> Rosario</a></li>
        <li><a onclick="showCity('Salta', -24.7833333, -65.4166667)"><i class="fa fa-angle-right"></i> Salta</a></li>
        <li><a onclick="showCity('San Miguel de Tucum\u00E1n', -26.8166667, -65.2166667)"><i class="fa fa-angle-right"></i> San Miguel de Tucum&aacute;n</a></li>
        <li><a onclick="showCity('Santa Fe de la Vera Cruz', -31.6333333, -60.7)"><i class="fa fa-angle-right"></i> Santa Fe de la Vera Cruz</a></li>
    </ol>
</li>
<li id="country-AU"><a class="country" onclick="toggleCountry('AU')"><img src="img/flag/AU.png" alt="Australia"/>Australia</a>
    <ol id="cities-AU">
        <li><a onclick="showCity('Adelaide', -34.9333333, 138.6)"><i class="fa fa-angle-right"></i> Adelaide</a></li>
        <li><a onclick="showCity('Brisbane', -27.4679357321901, 153.028092384338)"><i class="fa fa-angle-right"></i> Brisbane</a></li>
        <li><a onclick="showCity('Canberra', -35.2834624726481, 149.128074645996)"><i class="fa fa-angle-right"></i> Canberra</a></li>
        <li><a onclick="showCity('Gold Coast', -28.0, 153.4333333)"><i class="fa fa-angle-right"></i> Gold Coast</a></li>
        <li><a onclick="showCity('Melbourne', -37.8139965641595, 144.963322877884)"><i class="fa fa-angle-right"></i> Melbourne</a></li>
        <li><a onclick="showCity('Newcastle', -32.9271483615446, 151.776466369629)"><i class="fa fa-angle-right"></i> Newcastle</a></li>
        <li><a onclick="showCity('Perth', -31.9333333, 115.8333333)"><i class="fa fa-angle-right"></i> Perth</a></li>
        <li><a onclick="showCity('Sydney', -33.8678499639382, 151.207323074341)"><i class="fa fa-angle-right"></i> Sydney</a></li>
        <li><a onclick="showCity('Wollongong', -34.4333333, 150.8833333)"><i class="fa fa-angle-right"></i> Wollongong</a></li>
    </ol>
</li>
<li id="country-AT"><a class="country" onclick="toggleCountry('AT')"><img src="img/flag/AT.png" alt="Austria"/>Austria</a>
    <ol id="cities-AT">
        <li><a onclick="showCity('Vienna', 48.2084877601653, 16.3720750808716)"><i class="fa fa-angle-right"></i> Vienna</a></li>
    </ol>
</li>
<li id="country-BD"><a class="country" onclick="toggleCountry('BD')"><img src="img/flag/BD.png" alt="Bangladesh"/>Bangladesh</a>
    <ol id="cities-BD">
        <li><a onclick="showCity('Chittagong', 22.3330556, 91.8363889)"><i class="fa fa-angle-right"></i> Chittagong</a></li>
        <li><a onclick="showCity('Comilla', 23.4577778, 91.2044444)"><i class="fa fa-angle-right"></i> Comilla</a></li>
        <li><a onclick="showCity('Cox\'s B\u0101z\u0101r', 21.4422222, 91.9780556)"><i class="fa fa-angle-right"></i> Cox's B&#257;z&#257;r</a></li>
        <li><a onclick="showCity('Dhaka', 23.7230556, 90.4086111)"><i class="fa fa-angle-right"></i> Dhaka</a></li>
        <li><a onclick="showCity('Khulna', 22.8, 89.55)"><i class="fa fa-angle-right"></i> Khulna</a></li>
        <li><a onclick="showCity('Mymensingh', 24.75, 90.4)"><i class="fa fa-angle-right"></i> Mymensingh</a></li>
        <li><a onclick="showCity('Narsingdi', 23.9208333, 90.7180556)"><i class="fa fa-angle-right"></i> Narsingdi</a></li>
        <li><a onclick="showCity('R\u0101jsh\u0101hi', 24.3666667, 88.6)"><i class="fa fa-angle-right"></i> R&#257;jsh&#257;hi</a></li>
        <li><a onclick="showCity('Rangpur', 25.75, 89.25)"><i class="fa fa-angle-right"></i> Rangpur</a></li>
        <li><a onclick="showCity('Tungi', 23.89, 90.4058333)"><i class="fa fa-angle-right"></i> Tungi</a></li>
    </ol>
</li>
<li id="country-BE"><a class="country" onclick="toggleCountry('BE')"><img src="img/flag/BE.png" alt="Belgium"/>Belgium</a>
    <ol id="cities-BE">
        <li><a onclick="showCity('Antwerp', 51.2166667, 4.4166667)"><i class="fa fa-angle-right"></i> Antwerp</a></li>
        <li><a onclick="showCity('Brussels', 50.8465974826927, 4.35277462005615)"><i class="fa fa-angle-right"></i> Brussels</a></li>
    </ol>
</li>
<li id="country-BR"><a class="country" onclick="toggleCountry('BR')"><img src="img/flag/BR.png" alt="Brazil"/>Brazil</a>
    <ol id="cities-BR">
        <li><a onclick="showCity('Bel\u00E9m', -1.455833333, -48.50444444)"><i class="fa fa-angle-right"></i> Bel&eacute;m</a></li>
        <li><a onclick="showCity('Belo Horizonte', -19.92083333, -43.93777778)"><i class="fa fa-angle-right"></i> Belo Horizonte</a></li>
        <li><a onclick="showCity('Bras\u00EDlia', -15.77972, -47.92972)"><i class="fa fa-angle-right"></i> Bras&iacute;lia</a></li>
        <li><a onclick="showCity('Curitiba', -25.42777778, -49.27305556)"><i class="fa fa-angle-right"></i> Curitiba</a></li>
        <li><a onclick="showCity('Fortaleza', -3.717222222, -38.54305556)"><i class="fa fa-angle-right"></i> Fortaleza</a></li>
        <li><a onclick="showCity('Manaus', -3.101944444, -60.025)"><i class="fa fa-angle-right"></i> Manaus</a></li>
        <li><a onclick="showCity('Recife', -8.053888889, -34.88111111)"><i class="fa fa-angle-right"></i> Recife</a></li>
        <li><a onclick="showCity('Rio de Janeiro', -22.90277778, -43.2075)"><i class="fa fa-angle-right"></i> Rio de Janeiro</a></li>
        <li><a onclick="showCity('Salvador', -12.97111111, -38.51083333)"><i class="fa fa-angle-right"></i> Salvador</a></li>
        <li><a onclick="showCity('S\u00E3o Paulo', -23.5475, -46.63611111)"><i class="fa fa-angle-right"></i> S&atilde;o Paulo</a></li>
    </ol>
</li>
<li id="country-BF"><a class="country" onclick="toggleCountry('BF')"><img src="img/flag/BF.png" alt="Burkina Faso"/>Burkina Faso</a>
    <ol id="cities-BF">
        <li><a onclick="showCity('Bobo Dioulasso', 11.2, -4.3)"><i class="fa fa-angle-right"></i> Bobo Dioulasso</a></li>
        <li><a onclick="showCity('Ouagadougou', 12.3702778, -1.5247222)"><i class="fa fa-angle-right"></i> Ouagadougou</a></li>
    </ol>
</li>
<li id="country-KH"><a class="country" onclick="toggleCountry('KH')"><img src="img/flag/KH.png" alt="Cambodia"/>Cambodia</a>
    <ol id="cities-KH">
        <li><a onclick="showCity('Phnom Penh', 11.55, 104.9166667)"><i class="fa fa-angle-right"></i> Phnom Penh</a></li>
    </ol>
</li>
<li id="country-CM"><a class="country" onclick="toggleCountry('CM')"><img src="img/flag/CM.png" alt="Cameroon"/>Cameroon</a>
    <ol id="cities-CM">
        <li><a onclick="showCity('Bafoussam', 5.4666667, 10.4166667)"><i class="fa fa-angle-right"></i> Bafoussam</a></li>
        <li><a onclick="showCity('Bamenda', 5.9333333, 10.1666667)"><i class="fa fa-angle-right"></i> Bamenda</a></li>
        <li><a onclick="showCity('Douala', 4.0502778, 9.7)"><i class="fa fa-angle-right"></i> Douala</a></li>
        <li><a onclick="showCity('Garoua', 9.3, 13.4)"><i class="fa fa-angle-right"></i> Garoua</a></li>
        <li><a onclick="showCity('Kouss\u00E9ri', 12.0783333, 15.0308333)"><i class="fa fa-angle-right"></i> Kouss&eacute;ri</a></li>
        <li><a onclick="showCity('Maroua', 10.5955556, 14.3247222)"><i class="fa fa-angle-right"></i> Maroua</a></li>
        <li><a onclick="showCity('Mokolo', 10.7402778, 13.8027778)"><i class="fa fa-angle-right"></i> Mokolo</a></li>
        <li><a onclick="showCity('Yaound\u00E9', 3.8666667, 11.5166667)"><i class="fa fa-angle-right"></i> Yaound&eacute;</a></li>
    </ol>
</li>
<li id="country-CA"><a class="country" onclick="toggleCountry('CA')"><img src="img/flag/CA.png" alt="Canada"/>Canada</a>
    <ol id="cities-CA">
        <li><a onclick="showCity('Calgary', 51.050112282, -114.085285152)"><i class="fa fa-angle-right"></i> Calgary</a></li>
        <li><a onclick="showCity('Edmonton', 53.550135939, -113.468712219)"><i class="fa fa-angle-right"></i> Edmonton</a></li>
        <li><a onclick="showCity('Hamilton', 43.233408354, -79.949639869)"><i class="fa fa-angle-right"></i> Hamilton</a></li>
        <li><a onclick="showCity('Montr\u00E9al', 45.5088375, -73.587809)"><i class="fa fa-angle-right"></i> Montr&eacute;al</a></li>
        <li><a onclick="showCity('North York', 43.766813883, -79.416304667)"><i class="fa fa-angle-right"></i> North York</a></li>
        <li><a onclick="showCity('Ottawa', 45.4209405046496, -75.6902861595154)"><i class="fa fa-angle-right"></i> Ottawa</a></li>
        <li><a onclick="showCity('Qu\u00E9bec', 46.8122791164294, -71.2145376205444)"><i class="fa fa-angle-right"></i> Qu&eacute;bec</a></li>
        <li><a onclick="showCity('Toronto', 43.700113788, -79.416304194)"><i class="fa fa-angle-right"></i> Toronto</a></li>
        <li><a onclick="showCity('Vancouver', 49.249657393, -123.119340403)"><i class="fa fa-angle-right"></i> Vancouver</a></li>
        <li><a onclick="showCity('Winnipeg', 49.884398624, -97.147044715)"><i class="fa fa-angle-right"></i> Winnipeg</a></li>
    </ol>
</li>
<li id="country-TD"><a class="country" onclick="toggleCountry('TD')"><img src="img/flag/TD.png" alt="Chad"/>Chad</a>
    <ol id="cities-TD">
        <li><a onclick="showCity('N\'Djamena', 12.1130556, 15.0491667)"><i class="fa fa-angle-right"></i> N'Djamena</a></li>
    </ol>
</li>
<li id="country-CL"><a class="country" onclick="toggleCountry('CL')"><img src="img/flag/CL.png" alt="Chile"/>Chile</a>
    <ol id="cities-CL">
        <li><a onclick="showCity('Antofagasta', -23.65, -70.4)"><i class="fa fa-angle-right"></i> Antofagasta</a></li>
        <li><a onclick="showCity('Santiago', -33.4262838490987, -70.5665588378906)"><i class="fa fa-angle-right"></i> Santiago</a></li>
        <li><a onclick="showCity('Talcahuano', -36.7166667, -73.1166667)"><i class="fa fa-angle-right"></i> Talcahuano</a></li>
        <li><a onclick="showCity('Valpara\u00EDso', -33.0393201968221, -71.627254486084)"><i class="fa fa-angle-right"></i> Valpara&iacute;so</a></li>
        <li><a onclick="showCity('vi\u00F1a causi\u00F1o', -33.6166667, -70.5833333)"><i class="fa fa-angle-right"></i> vi&ntilde;a causi&ntilde;o</a></li>
        <li><a onclick="showCity('Vi\u00F1a del Mar', -33.0245688893063, -71.5518307685852)"><i class="fa fa-angle-right"></i> Vi&ntilde;a del Mar</a></li>
    </ol>
</li>
<li id="country-CN"><a class="country" onclick="toggleCountry('CN')"><img src="img/flag/CN.png" alt="China"/>China</a>
    <ol id="cities-CN">
        <li><a onclick="showCity('Beijing', 39.9074977414405, 116.397228240967)"><i class="fa fa-angle-right"></i> Beijing</a></li>
        <li><a onclick="showCity('Chengdu', 30.6666667, 104.0666667)"><i class="fa fa-angle-right"></i> Chengdu</a></li>
        <li><a onclick="showCity('Chongqing', 29.5627778, 106.5527778)"><i class="fa fa-angle-right"></i> Chongqing</a></li>
        <li><a onclick="showCity('Kaifeng', 34.7911111, 114.3483333)"><i class="fa fa-angle-right"></i> Kaifeng</a></li>
        <li><a onclick="showCity('Nanchong', 30.8, 106.0666667)"><i class="fa fa-angle-right"></i> Nanchong</a></li>
        <li><a onclick="showCity('Shanghai', 31.2222222, 121.4580556)"><i class="fa fa-angle-right"></i> Shanghai</a></li>
        <li><a onclick="showCity('Tai\'an', 36.1852778, 117.12)"><i class="fa fa-angle-right"></i> Tai'an</a></li>
        <li><a onclick="showCity('Tianjin', 39.1422222, 117.1766667)"><i class="fa fa-angle-right"></i> Tianjin</a></li>
        <li><a onclick="showCity('Wuhan', 30.5833333, 114.2666667)"><i class="fa fa-angle-right"></i> Wuhan</a></li>
        <li><a onclick="showCity('Zhumadian', 32.9794444, 114.0294444)"><i class="fa fa-angle-right"></i> Zhumadian</a></li>
    </ol>
</li>
<li id="country-CO"><a class="country" onclick="toggleCountry('CO')"><img src="img/flag/CO.png" alt="Colombia"/>Colombia</a>
    <ol id="cities-CO">
        <li><a onclick="showCity('Barranquilla', 10.9638889, -74.7963889)"><i class="fa fa-angle-right"></i> Barranquilla</a></li>
        <li><a onclick="showCity('Bogot\u00E1', 4.6, -74.0833333)"><i class="fa fa-angle-right"></i> Bogot&aacute;</a></li>
        <li><a onclick="showCity('Bucaramanga', 7.1297222, -73.1258333)"><i class="fa fa-angle-right"></i> Bucaramanga</a></li>
        <li><a onclick="showCity('Cali', 3.4372222, -76.5225)"><i class="fa fa-angle-right"></i> Cali</a></li>
        <li><a onclick="showCity('Cartagena', 10.3997222, -75.5144444)"><i class="fa fa-angle-right"></i> Cartagena</a></li>
        <li><a onclick="showCity('C\u00FAcuta', 7.8833333, -72.5052778)"><i class="fa fa-angle-right"></i> C&uacute;cuta</a></li>
        <li><a onclick="showCity('Ibagu\u00E9', 4.4388889, -75.2322222)"><i class="fa fa-angle-right"></i> Ibagu&eacute;</a></li>
        <li><a onclick="showCity('Medell\u00EDn', 6.2913889, -75.5361111)"><i class="fa fa-angle-right"></i> Medell&iacute;n</a></li>
        <li><a onclick="showCity('Pereira', 4.8133333, -75.6961111)"><i class="fa fa-angle-right"></i> Pereira</a></li>
        <li><a onclick="showCity('Santa Marta', 11.2472222, -74.2016667)"><i class="fa fa-angle-right"></i> Santa Marta</a></li>
    </ol>
</li>
<li id="country-CU"><a class="country" onclick="toggleCountry('CU')"><img src="img/flag/CU.png" alt="Cuba"/>Cuba</a>
    <ol id="cities-CU">
        <li><a onclick="showCity('Camag\u00FCey', 21.3808333, -77.9169444)"><i class="fa fa-angle-right"></i> Camag&uuml;ey</a></li>
        <li><a onclick="showCity('Guant\u00E1namo', 20.1444444, -75.2091667)"><i class="fa fa-angle-right"></i> Guant&aacute;namo</a></li>
        <li><a onclick="showCity('Havana', 23.1319444, -82.3641667)"><i class="fa fa-angle-right"></i> Havana</a></li>
        <li><a onclick="showCity('Holgu\u00EDn', 20.8872222, -76.2630556)"><i class="fa fa-angle-right"></i> Holgu&iacute;n</a></li>
        <li><a onclick="showCity('Santa Clara', 22.4, -79.9666667)"><i class="fa fa-angle-right"></i> Santa Clara</a></li>
        <li><a onclick="showCity('Santiago de Cuba', 20.0247222, -75.8219444)"><i class="fa fa-angle-right"></i> Santiago de Cuba</a></li>
    </ol>
</li>
<li id="country-CZ"><a class="country" onclick="toggleCountry('CZ')"><img src="img/flag/CZ.png" alt="Czech Republic"/>Czech Republic</a>
    <ol id="cities-CZ">
        <li><a onclick="showCity('Brno', 49.1952226735148, 16.6079592704773)"><i class="fa fa-angle-right"></i> Brno</a></li>
        <li><a onclick="showCity('Ostrava', 49.8346452000378, 18.2820440791588)"><i class="fa fa-angle-right"></i> Ostrava</a></li>
        <li><a onclick="showCity('Praha', 50.0878367932108, 14.4241322001241)"><i class="fa fa-angle-right"></i> Praha</a></li>
    </ol>
</li>
<li id="country-CD"><a class="country" onclick="toggleCountry('CD')"><img src="img/flag/CD.png" alt="Democratic Republic of the Congo"/>Democratic Republic of the Congo</a>
    <ol id="cities-CD">
        <li><a onclick="showCity('Kananga', -5.8958333, 22.4177778)"><i class="fa fa-angle-right"></i> Kananga</a></li>
        <li><a onclick="showCity('Kinshasa', -4.3297222, 15.315)"><i class="fa fa-angle-right"></i> Kinshasa</a></li>
        <li><a onclick="showCity('Kisangani', 0.5166667, 25.2)"><i class="fa fa-angle-right"></i> Kisangani</a></li>
        <li><a onclick="showCity('Kolwezi', -10.7166667, 25.4725)"><i class="fa fa-angle-right"></i> Kolwezi</a></li>
        <li><a onclick="showCity('Likasi', -10.9813889, 26.7333333)"><i class="fa fa-angle-right"></i> Likasi</a></li>
        <li><a onclick="showCity('Lubumbashi', -11.6666667, 27.4666667)"><i class="fa fa-angle-right"></i> Lubumbashi</a></li>
        <li><a onclick="showCity('Masina', -4.3836111, 15.3913889)"><i class="fa fa-angle-right"></i> Masina</a></li>
        <li><a onclick="showCity('Mbuji-Mayi', -6.15, 23.6)"><i class="fa fa-angle-right"></i> Mbuji-Mayi</a></li>
        <li><a onclick="showCity('Tshikapa', -6.4166667, 20.8)"><i class="fa fa-angle-right"></i> Tshikapa</a></li>
    </ol>
</li>
<li id="country-DK"><a class="country" onclick="toggleCountry('DK')"><img src="img/flag/DK.png" alt="Denmark"/>Denmark</a>
    <ol id="cities-DK">
        <li><a onclick="showCity('Copenhagen', 55.6776812020993, 12.5709342956543)"><i class="fa fa-angle-right"></i> Copenhagen</a></li>
    </ol>
</li>
<li id="country-EC"><a class="country" onclick="toggleCountry('EC')"><img src="img/flag/EC.png" alt="Ecuador"/>Ecuador</a>
    <ol id="cities-EC">
        <li><a onclick="showCity('Cuenca', -2.8833333, -78.9833333)"><i class="fa fa-angle-right"></i> Cuenca</a></li>
        <li><a onclick="showCity('Guayaquil', -2.1666667, -79.9)"><i class="fa fa-angle-right"></i> Guayaquil</a></li>
        <li><a onclick="showCity('Quito', -0.2166667, -78.5)"><i class="fa fa-angle-right"></i> Quito</a></li>
    </ol>
</li>
<li id="country-EG"><a class="country" onclick="toggleCountry('EG')"><img src="img/flag/EG.png" alt="Egypt"/>Egypt</a>
    <ol id="cities-EG">
        <li><a onclick="showCity('Al J\u012Bzah', 30.0086111, 31.2122222)"><i class="fa fa-angle-right"></i> Al J&#299;zah</a></li>
        <li><a onclick="showCity('Al Ma\u1E29allah al Kubr\u00E1', 30.9761111, 31.1669444)"><i class="fa fa-angle-right"></i> Al Ma&#7721;allah al Kubr&aacute;</a></li>
        <li><a onclick="showCity('Al Man\u015F\u016Brah', 31.0430556, 31.3766667)"><i class="fa fa-angle-right"></i> Al Man&#351;&#363;rah</a></li>
        <li><a onclick="showCity('Alexandria', 31.1980556, 29.9191667)"><i class="fa fa-angle-right"></i> Alexandria</a></li>
        <li><a onclick="showCity('Asy\u016B\u0163', 27.1827778, 31.1827778)"><i class="fa fa-angle-right"></i> Asy&#363;&#355;</a></li>
        <li><a onclick="showCity('B\u016Br Sa`\u012Bd', 31.2597699873943, 32.3004913330078)"><i class="fa fa-angle-right"></i> B&#363;r Sa`&#299;d</a></li>
        <li><a onclick="showCity('Cairo', 30.05, 31.25)"><i class="fa fa-angle-right"></i> Cairo</a></li>
        <li><a onclick="showCity('Luxor', 25.6989270387148, 32.6420974731445)"><i class="fa fa-angle-right"></i> Luxor</a></li>
        <li><a onclick="showCity('Suez', 29.9666667, 32.55)"><i class="fa fa-angle-right"></i> Suez</a></li>
        <li><a onclick="showCity('\u0162an\u0163\u0101', 30.7911111, 30.9980556)">&#354;an&#355;&#257;</a></li>
    </ol>
</li>
<li id="country-ET"><a class="country" onclick="toggleCountry('ET')"><img src="img/flag/ET.png" alt="Ethiopia"/>Ethiopia</a>
    <ol id="cities-ET">
        <li><a onclick="showCity('Addis Ababa', 9.0333333, 38.7)"><i class="fa fa-angle-right"></i> Addis Ababa</a></li>
        <li><a onclick="showCity('Dir\u0113 Dawa', 9.5930556, 41.8661111)"><i class="fa fa-angle-right"></i> Dir&#275; Dawa</a></li>
    </ol>
</li>
<li id="country-FI"><a class="country" onclick="toggleCountry('FI')"><img src="img/flag/FI.png" alt="Finland"/>Finland</a>
    <ol id="cities-FI">
        <li><a onclick="showCity('Helsinki', 60.1755556, 24.9341667)"><i class="fa fa-angle-right"></i> Helsinki</a></li>
    </ol>
</li>
<li id="country-FR"><a class="country" onclick="toggleCountry('FR')"><img src="img/flag/FR.png" alt="France"/>France</a>
    <ol id="cities-FR">
        <li><a onclick="showCity('Lyon', 45.75, 4.85)"><i class="fa fa-angle-right"></i> Lyon</a></li>
        <li><a onclick="showCity('Marseille', 43.3, 5.4)"><i class="fa fa-angle-right"></i> Marseille</a></li>
        <li><a onclick="showCity('Nantes', 47.2172508193978, -1.55336380004883)"><i class="fa fa-angle-right"></i> Nantes</a></li>
        <li><a onclick="showCity('Nice', 43.703126104186, 7.26608276367188)"><i class="fa fa-angle-right"></i> Nice</a></li>
        <li><a onclick="showCity('Paris', 48.85341, 2.3488)"><i class="fa fa-angle-right"></i> Paris</a></li>
        <li><a onclick="showCity('Strasbourg', 48.5833333, 7.75)"><i class="fa fa-angle-right"></i> Strasbourg</a></li>
        <li><a onclick="showCity('Toulouse', 43.6042618680962, 1.44367218017578)"><i class="fa fa-angle-right"></i> Toulouse</a></li>
    </ol>
</li>
<li id="country-DE"><a class="country" onclick="toggleCountry('DE')"><img src="img/flag/DE.png" alt="Germany"/>Germany</a>
    <ol id="cities-DE">
        <li><a onclick="showCity('Berlin', 52.5166667, 13.4)"><i class="fa fa-angle-right"></i> Berlin</a></li>
        <li><a onclick="showCity('Bremen', 53.075155679403, 8.80777359008789)"><i class="fa fa-angle-right"></i> Bremen</a></li>
        <li><a onclick="showCity('Dortmund', 51.5166667, 7.45)"><i class="fa fa-angle-right"></i> Dortmund</a></li>
        <li><a onclick="showCity('D\u00FCsseldorf', 51.2217225723832, 6.77616119384766)"><i class="fa fa-angle-right"></i> D&uuml;sseldorf</a></li>
        <li><a onclick="showCity('Essen', 51.45, 7.0166667)"><i class="fa fa-angle-right"></i> Essen</a></li>
        <li><a onclick="showCity('Frankfurt am Main', 50.1166667, 8.6833333)"><i class="fa fa-angle-right"></i> Frankfurt am Main</a></li>
        <li><a onclick="showCity('Hamburg', 53.55, 10.0)"><i class="fa fa-angle-right"></i> Hamburg</a></li>
        <li><a onclick="showCity('K\u00F6ln', 50.9333333, 6.95)"><i class="fa fa-angle-right"></i> K&ouml;ln</a></li>
        <li><a onclick="showCity('M\u00FCnchen', 48.1374325498109, 11.5754914283752)"><i class="fa fa-angle-right"></i> M&uuml;nchen</a></li>
        <li><a onclick="showCity('Stuttgart', 48.7823242931971, 9.17701721191406)"><i class="fa fa-angle-right"></i> Stuttgart</a></li>
    </ol>
</li>
<li id="country-GH"><a class="country" onclick="toggleCountry('GH')"><img src="img/flag/GH.png" alt="Ghana"/>Ghana</a>
    <ol id="cities-GH">
        <li><a onclick="showCity('Accra', 5.55, -0.2166667)"><i class="fa fa-angle-right"></i> Accra</a></li>
        <li><a onclick="showCity('Kumasi', 6.6833333, -1.6166667)"><i class="fa fa-angle-right"></i> Kumasi</a></li>
        <li><a onclick="showCity('Tamale', 9.4, -0.8333333)"><i class="fa fa-angle-right"></i> Tamale</a></li>
    </ol>
</li>
<li id="country-GR"><a class="country" onclick="toggleCountry('GR')"><img src="img/flag/GR.png" alt="Greece"/>Greece</a>
    <ol id="cities-GR">
        <li><a onclick="showCity('Athens', 37.9833333, 23.7333333)"><i class="fa fa-angle-right"></i> Athens</a></li>
        <li><a onclick="showCity('Thessalon\u00EDki', 40.6402778, 22.9438889)"><i class="fa fa-angle-right"></i> Thessalon&iacute;ki</a></li>
    </ol>
</li>
<li id="country-GT"><a class="country" onclick="toggleCountry('GT')"><img src="img/flag/GT.png" alt="Guatemala"/>Guatemala</a>
    <ol id="cities-GT">
        <li><a onclick="showCity('Guatemala City', 14.62111, -90.52694)"><i class="fa fa-angle-right"></i> Guatemala City</a></li>
        <li><a onclick="showCity('Mixco', 14.6333333, -90.6063889)"><i class="fa fa-angle-right"></i> Mixco</a></li>
        <li><a onclick="showCity('Villa Nueva', 14.5269444, -90.5875)"><i class="fa fa-angle-right"></i> Villa Nueva</a></li>
    </ol>
</li>
<li id="country-GN"><a class="country" onclick="toggleCountry('GN')"><img src="img/flag/GN.png" alt="Guinea"/>Guinea</a>
    <ol id="cities-GN">
        <li><a onclick="showCity('Camayenne', 9.535, -13.6877778)"><i class="fa fa-angle-right"></i> Camayenne</a></li>
        <li><a onclick="showCity('Conakry', 9.5091667, -13.7122222)"><i class="fa fa-angle-right"></i> Conakry</a></li>
    </ol>
</li>
<li id="country-IN"><a class="country" onclick="toggleCountry('IN')"><img src="img/flag/IN.png" alt="India"/>India</a>
    <ol id="cities-IN">
        <li><a onclick="showCity('Ahmad\u0101b\u0101d', 23.0333333, 72.6166667)"><i class="fa fa-angle-right"></i> Ahmad&#257;b&#257;d</a></li>
        <li><a onclick="showCity('Bengaluru', 12.9762266976805, 77.6032912731171)"><i class="fa fa-angle-right"></i> Bengaluru</a></li>
        <li><a onclick="showCity('Calcutta', 22.5697222, 88.3697222)"><i class="fa fa-angle-right"></i> Calcutta</a></li>
        <li><a onclick="showCity('Chennai', 13.0878385345075, 80.2784729003906)"><i class="fa fa-angle-right"></i> Chennai</a></li>
        <li><a onclick="showCity('Delhi', 28.6666667, 77.2166667)"><i class="fa fa-angle-right"></i> Delhi</a></li>
        <li><a onclick="showCity('Hyder\u0101b\u0101d', 17.3752778, 78.4744444)"><i class="fa fa-angle-right"></i> Hyder&#257;b&#257;d</a></li>
        <li><a onclick="showCity('K\u0101npur', 26.4666667, 80.35)"><i class="fa fa-angle-right"></i> K&#257;npur</a></li>
        <li><a onclick="showCity('Mumbai', 19.0144100168904, 72.8479385375977)"><i class="fa fa-angle-right"></i> Mumbai</a></li>
        <li><a onclick="showCity('Pune', 18.5195742288075, 73.8553547859192)"><i class="fa fa-angle-right"></i> Pune</a></li>
        <li><a onclick="showCity('S\u016Brat', 21.1666667, 72.8333333)"><i class="fa fa-angle-right"></i> S&#363;rat</a></li>
    </ol>
</li>
<li id="country-ID"><a class="country" onclick="toggleCountry('ID')"><img src="img/flag/ID.png" alt="Indonesia"/>Indonesia</a>
    <ol id="cities-ID">
        <li><a onclick="showCity('Bandung', -6.9127778, 107.6205556)"><i class="fa fa-angle-right"></i> Bandung</a></li>
        <li><a onclick="showCity('Bekasi', -6.2366667, 106.9919444)"><i class="fa fa-angle-right"></i> Bekasi</a></li>
        <li><a onclick="showCity('Depok', -6.4, 106.8186111)"><i class="fa fa-angle-right"></i> Depok</a></li>
        <li><a onclick="showCity('Jakarta', -6.1744444, 106.8294444)"><i class="fa fa-angle-right"></i> Jakarta</a></li>
        <li><a onclick="showCity('Makassar', -5.1463889, 119.4386111)"><i class="fa fa-angle-right"></i> Makassar</a></li>
        <li><a onclick="showCity('Medan', 3.5833333, 98.6666667)"><i class="fa fa-angle-right"></i> Medan</a></li>
        <li><a onclick="showCity('Palembang', -2.91672533277337, 104.745798110962)"><i class="fa fa-angle-right"></i> Palembang</a></li>
        <li><a onclick="showCity('Semarang', -6.9666667, 110.4166667)"><i class="fa fa-angle-right"></i> Semarang</a></li>
        <li><a onclick="showCity('Surabaya', -7.2491667, 112.7508333)"><i class="fa fa-angle-right"></i> Surabaya</a></li>
        <li><a onclick="showCity('Tangerang', -6.1780556, 106.63)"><i class="fa fa-angle-right"></i> Tangerang</a></li>
    </ol>
</li>
<li id="country-IQ"><a class="country" onclick="toggleCountry('IQ')"><img src="img/flag/IQ.png" alt="Iraq"/>Iraq</a>
    <ol id="cities-IQ">
        <li><a onclick="showCity('Ab\u016B Ghurayb', 33.3083333, 44.185)"><i class="fa fa-angle-right"></i> Ab&#363; Ghurayb</a></li>
        <li><a onclick="showCity('Al Ba\u015Frah', 30.4941667, 47.8191667)"><i class="fa fa-angle-right"></i> Al Ba&#351;rah</a></li>
        <li><a onclick="showCity('Al Ba\u015Frah al Qad\u012Bmah', 30.4955556, 47.8175)"><i class="fa fa-angle-right"></i> Al Ba&#351;rah al Qad&#299;mah</a></li>
        <li><a onclick="showCity('Al Maw\u015Fil al Jad\u012Bdah', 36.3291667, 43.0861111)"><i class="fa fa-angle-right"></i> Al Maw&#351;il al Jad&#299;dah</a></li>
        <li><a onclick="showCity('An Najaf', 31.9961111, 44.3147222)"><i class="fa fa-angle-right"></i> An Najaf</a></li>
        <li><a onclick="showCity('Arb\u012Bl', 36.19, 44.0088889)"><i class="fa fa-angle-right"></i> Arb&#299;l</a></li>
        <li><a onclick="showCity('As Sulaym\u0101n\u012Byah', 35.5616667, 45.4408333)"><i class="fa fa-angle-right"></i> As Sulaym&#257;n&#299;yah</a></li>
        <li><a onclick="showCity('Baghdad', 33.3386111, 44.3938889)"><i class="fa fa-angle-right"></i> Baghdad</a></li>
        <li><a onclick="showCity('Kirkuk', 35.4680556, 44.3922222)"><i class="fa fa-angle-right"></i> Kirkuk</a></li>
        <li><a onclick="showCity('Mosul', 36.335, 43.1188889)"><i class="fa fa-angle-right"></i> Mosul</a></li>
    </ol>
</li>
<li id="country-IT"><a class="country" onclick="toggleCountry('IT')"><img src="img/flag/IT.png" alt="Italy"/>Italy</a>
    <ol id="cities-IT">
        <li><a onclick="showCity('Bari', 41.117734, 16.851185)"><i class="fa fa-angle-right"></i> Bari</a></li>
        <li><a onclick="showCity('Bologna', 44.4938114812334, 11.3387489318848)"><i class="fa fa-angle-right"></i> Bologna</a></li>
        <li><a onclick="showCity('Catania', 37.5021339578821, 15.0871896743774)"><i class="fa fa-angle-right"></i> Catania</a></li>
        <li><a onclick="showCity('Florence', 43.7666667, 11.25)"><i class="fa fa-angle-right"></i> Florence</a></li>
        <li><a onclick="showCity('Genova', 44.4063162526614, 8.93385887145996)"><i class="fa fa-angle-right"></i> Genova</a></li>
        <li><a onclick="showCity('Milano', 45.4642693810258, 9.1895055770874)"><i class="fa fa-angle-right"></i> Milano</a></li>
        <li><a onclick="showCity('Napoli', 40.8333333, 14.25)"><i class="fa fa-angle-right"></i> Napoli</a></li>
        <li><a onclick="showCity('Palermo', 38.115819808927, 13.3597612380981)"><i class="fa fa-angle-right"></i> Palermo</a></li>
        <li><a onclick="showCity('Roma', 41.9, 12.4833333)"><i class="fa fa-angle-right"></i> Roma</a></li>
        <li><a onclick="showCity('Torino', 45.0704898496472, 7.68682479858398)"><i class="fa fa-angle-right"></i> Torino</a></li>
    </ol>
</li>
<li id="country-CI"><a class="country" onclick="toggleCountry('CI')"><img src="img/flag/CI.png" alt="Ivory Coast"/>Ivory Coast</a>
    <ol id="cities-CI">
        <li><a onclick="showCity('Abidjan', 5.3411111, -4.0280556)"><i class="fa fa-angle-right"></i> Abidjan</a></li>
        <li><a onclick="showCity('Abobo', 5.4188889, -4.0205556)"><i class="fa fa-angle-right"></i> Abobo</a></li>
        <li><a onclick="showCity('Bouak\u00E9', 7.6833333, -5.0330556)"><i class="fa fa-angle-right"></i> Bouak&eacute;</a></li>
    </ol>
</li>
<li id="country-JP"><a class="country" onclick="toggleCountry('JP')"><img src="img/flag/JP.png" alt="Japan"/>Japan</a>
    <ol id="cities-JP">
        <li><a onclick="showCity('Fukuoka', 33.5833333, 130.4)"><i class="fa fa-angle-right"></i> Fukuoka</a></li>
        <li><a onclick="showCity('Kawasaki', 35.5205556, 139.7172222)"><i class="fa fa-angle-right"></i> Kawasaki</a></li>
        <li><a onclick="showCity('K\u014Dbe', 34.6833333, 135.1666667)"><i class="fa fa-angle-right"></i> K&#333;be</a></li>
        <li><a onclick="showCity('Kyoto', 35.02106999142, 135.75385093689)"><i class="fa fa-angle-right"></i> Kyoto</a></li>
        <li><a onclick="showCity('Nagoya', 35.1814727966958, 136.906409561634)"><i class="fa fa-angle-right"></i> Nagoya</a></li>
        <li><a onclick="showCity('\u014Csaka', 34.6937398415059, 135.502181947231)">&#332;saka</a></li>
        <li><a onclick="showCity('Saitama', 35.9080659046769, 139.656572341919)"><i class="fa fa-angle-right"></i> Saitama</a></li>
        <li><a onclick="showCity('Sapporo', 43.0547222, 141.3538889)"><i class="fa fa-angle-right"></i> Sapporo</a></li>
        <li><a onclick="showCity('Tokyo', 35.6895265930799, 139.691677093506)"><i class="fa fa-angle-right"></i> Tokyo</a></li>
        <li><a onclick="showCity('Yokohama', 35.45, 139.65)"><i class="fa fa-angle-right"></i> Yokohama</a></li>
    </ol>
</li>
<li id="country-KZ"><a class="country" onclick="toggleCountry('KZ')"><img src="img/flag/KZ.png" alt="Kazakhstan"/>Kazakhstan</a>
    <ol id="cities-KZ">
        <li><a onclick="showCity('Almaty', 43.25, 76.95)"><i class="fa fa-angle-right"></i> Almaty</a></li>
        <li><a onclick="showCity('Astana', 51.1811111, 71.4277778)"><i class="fa fa-angle-right"></i> Astana</a></li>
        <li><a onclick="showCity('Karagandy', 50.0666667, 54.8666667)"><i class="fa fa-angle-right"></i> Karagandy</a></li>
        <li><a onclick="showCity('Kyzylorda', 44.8527778, 65.5091667)"><i class="fa fa-angle-right"></i> Kyzylorda</a></li>
        <li><a onclick="showCity('Pavlodar', 52.3, 76.95)"><i class="fa fa-angle-right"></i> Pavlodar</a></li>
        <li><a onclick="showCity('Qaraghandy', 49.7988889, 73.0994444)"><i class="fa fa-angle-right"></i> Qaraghandy</a></li>
        <li><a onclick="showCity('Semipalatinsk', 50.4111111, 80.2275)"><i class="fa fa-angle-right"></i> Semipalatinsk</a></li>
        <li><a onclick="showCity('Shymkent', 42.3, 69.6)"><i class="fa fa-angle-right"></i> Shymkent</a></li>
        <li><a onclick="showCity('Taraz', 42.9, 71.3666667)"><i class="fa fa-angle-right"></i> Taraz</a></li>
        <li><a onclick="showCity('Ust\'-Kamenogorsk', 49.9788889, 82.6102778)"><i class="fa fa-angle-right"></i> Ust'-Kamenogorsk</a></li>
    </ol>
</li>
<li id="country-KE"><a class="country" onclick="toggleCountry('KE')"><img src="img/flag/KE.png" alt="Kenya"/>Kenya</a>
    <ol id="cities-KE">
        <li><a onclick="showCity('Mombasa', -4.05, 39.6666667)"><i class="fa fa-angle-right"></i> Mombasa</a></li>
        <li><a onclick="showCity('Nairobi', -1.2833333, 36.8166667)"><i class="fa fa-angle-right"></i> Nairobi</a></li>
        <li><a onclick="showCity('Nakuru', -0.2833333, 36.0666667)"><i class="fa fa-angle-right"></i> Nakuru</a></li>
    </ol>
</li>
<li id="country-MG"><a class="country" onclick="toggleCountry('MG')"><img src="img/flag/MG.png" alt="Madagascar"/>Madagascar</a>
    <ol id="cities-MG">
        <li><a onclick="showCity('Antananarivo', -18.9166667, 47.5166667)"><i class="fa fa-angle-right"></i> Antananarivo</a></li>
    </ol>
</li>
<li id="country-MW"><a class="country" onclick="toggleCountry('MW')"><img src="img/flag/MW.png" alt="Malawi"/>Malawi</a>
    <ol id="cities-MW">
        <li><a onclick="showCity('Blantyre', -15.7833333, 35.0)"><i class="fa fa-angle-right"></i> Blantyre</a></li>
        <li><a onclick="showCity('Lilongwe', -13.9833333, 33.7833333)"><i class="fa fa-angle-right"></i> Lilongwe</a></li>
    </ol>
</li>
<li id="country-MY"><a class="country" onclick="toggleCountry('MY')"><img src="img/flag/MY.png" alt="Malaysia"/>Malaysia</a>
    <ol id="cities-MY">
        <li><a onclick="showCity('Ipoh', 4.5833333, 101.0833333)"><i class="fa fa-angle-right"></i> Ipoh</a></li>
        <li><a onclick="showCity('Johor Bahru', 1.4666667, 103.75)"><i class="fa fa-angle-right"></i> Johor Bahru</a></li>
        <li><a onclick="showCity('Kampung Baru Subang', 3.15, 101.5333333)"><i class="fa fa-angle-right"></i> Kampung Baru Subang</a></li>
        <li><a onclick="showCity('Klang', 3.0333333, 101.45)"><i class="fa fa-angle-right"></i> Klang</a></li>
        <li><a onclick="showCity('Kota Kinabalu', 5.9833333, 116.0666667)"><i class="fa fa-angle-right"></i> Kota Kinabalu</a></li>
        <li><a onclick="showCity('Kuala Lumpur', 3.1666667, 101.7)"><i class="fa fa-angle-right"></i> Kuala Lumpur</a></li>
        <li><a onclick="showCity('Kuching', 1.55, 110.3333333)"><i class="fa fa-angle-right"></i> Kuching</a></li>
        <li><a onclick="showCity('Petaling Jaya', 3.10726328338124, 101.606712341309)"><i class="fa fa-angle-right"></i> Petaling Jaya</a></li>
        <li><a onclick="showCity('Sandakan', 5.8333333, 118.1166667)"><i class="fa fa-angle-right"></i> Sandakan</a></li>
        <li><a onclick="showCity('Shah Alam', 3.08506558481303, 101.53281211853)"><i class="fa fa-angle-right"></i> Shah Alam</a></li>
    </ol>
</li>
<li id="country-ML"><a class="country" onclick="toggleCountry('ML')"><img src="img/flag/ML.png" alt="Mali"/>Mali</a>
    <ol id="cities-ML">
        <li><a onclick="showCity('Bamako', 12.65, -8.0)"><i class="fa fa-angle-right"></i> Bamako</a></li>
    </ol>
</li>
<li id="country-MX"><a class="country" onclick="toggleCountry('MX')"><img src="img/flag/MX.png" alt="Mexico"/>Mexico</a>
    <ol id="cities-MX">
        <li><a onclick="showCity('Ciudad Ju\u00E1rez', 31.7333333, -106.4833333)"><i class="fa fa-angle-right"></i> Ciudad Ju&aacute;rez</a></li>
        <li><a onclick="showCity('Ciudad Nezahualc\u00F3yotl', 19.4136111, -99.0330556)"><i class="fa fa-angle-right"></i> Ciudad Nezahualc&oacute;yotl</a></li>
        <li><a onclick="showCity('Ecatepec', 19.6011111, -99.0525)"><i class="fa fa-angle-right"></i> Ecatepec</a></li>
        <li><a onclick="showCity('Guadalajara', 20.6666667, -103.3333333)"><i class="fa fa-angle-right"></i> Guadalajara</a></li>
        <li><a onclick="showCity('Le\u00F3n', 21.1166667, -101.6666667)"><i class="fa fa-angle-right"></i> Le&oacute;n</a></li>
        <li><a onclick="showCity('Mexico City', 19.4341667, -99.1386111)"><i class="fa fa-angle-right"></i> Mexico City</a></li>
        <li><a onclick="showCity('Monterrey', 25.6666667, -100.3166667)"><i class="fa fa-angle-right"></i> Monterrey</a></li>
        <li><a onclick="showCity('Puebla de Zaragoza', 19.05, -98.2)"><i class="fa fa-angle-right"></i> Puebla de Zaragoza</a></li>
        <li><a onclick="showCity('Tijuana', 32.5333333, -117.0166667)"><i class="fa fa-angle-right"></i> Tijuana</a></li>
        <li><a onclick="showCity('Zapopan', 20.7166667, -103.4)"><i class="fa fa-angle-right"></i> Zapopan</a></li>
    </ol>
</li>
<li id="country-MA"><a class="country" onclick="toggleCountry('MA')"><img src="img/flag/MA.png" alt="Morocco"/>Morocco</a>
    <ol id="cities-MA">
        <li><a onclick="showCity('Agadir', 30.4, -9.6)"><i class="fa fa-angle-right"></i> Agadir</a></li>
        <li><a onclick="showCity('Casablanca', 33.59, -7.61)"><i class="fa fa-angle-right"></i> Casablanca</a></li>
        <li><a onclick="showCity('F\u00E8s', 34.05, -4.98)"><i class="fa fa-angle-right"></i> F&egrave;s</a></li>
        <li><a onclick="showCity('Kenitra', 34.26, -6.57)"><i class="fa fa-angle-right"></i> Kenitra</a></li>
        <li><a onclick="showCity('Marrakech', 31.63, -8.0)"><i class="fa fa-angle-right"></i> Marrakech</a></li>
        <li><a onclick="showCity('Mekn\u00E8s', 33.9, -5.55)"><i class="fa fa-angle-right"></i> Mekn&egrave;s</a></li>
        <li><a onclick="showCity('Oujda', 34.68, -1.91)"><i class="fa fa-angle-right"></i> Oujda</a></li>
        <li><a onclick="showCity('Rabat', 34.02, -6.83)"><i class="fa fa-angle-right"></i> Rabat</a></li>
        <li><a onclick="showCity('Tangier', 35.78, -5.81)"><i class="fa fa-angle-right"></i> Tangier</a></li>
        <li><a onclick="showCity('Tetouan', 35.57, -5.37)"><i class="fa fa-angle-right"></i> Tetouan</a></li>
    </ol>
</li>
<li id="country-MZ"><a class="country" onclick="toggleCountry('MZ')"><img src="img/flag/MZ.png" alt="Mozambique"/>Mozambique</a>
    <ol id="cities-MZ">
        <li><a onclick="showCity('Beira', -19.8436111, 34.8388889)"><i class="fa fa-angle-right"></i> Beira</a></li>
        <li><a onclick="showCity('Chimoio', -19.1163889, 33.4833333)"><i class="fa fa-angle-right"></i> Chimoio</a></li>
        <li><a onclick="showCity('Maputo', -25.9652778, 32.5891667)"><i class="fa fa-angle-right"></i> Maputo</a></li>
        <li><a onclick="showCity('Matola', -25.9622222, 32.4588889)"><i class="fa fa-angle-right"></i> Matola</a></li>
        <li><a onclick="showCity('Nampula', -15.1197222, 39.2647222)"><i class="fa fa-angle-right"></i> Nampula</a></li>
    </ol>
</li>
<li id="country-NP"><a class="country" onclick="toggleCountry('NP')"><img src="img/flag/NP.png" alt="Nepal"/>Nepal</a>
    <ol id="cities-NP">
        <li><a onclick="showCity('Kathmandu', 27.7166667, 85.3166667)"><i class="fa fa-angle-right"></i> Kathmandu</a></li>
    </ol>
</li>
<li id="country-NL"><a class="country" onclick="toggleCountry('NL')"><img src="img/flag/NL.png" alt="Netherlands"/>Netherlands</a>
    <ol id="cities-NL">
        <li><a onclick="showCity('Amsterdam', 52.3730839945403, 4.89990234375)"><i class="fa fa-angle-right"></i> Amsterdam</a></li>
        <li><a onclick="showCity('Den Haag', 52.076667, 4.298611)"><i class="fa fa-angle-right"></i> Den Haag</a></li>
        <li><a onclick="showCity('Rotterdam', 51.9225, 4.479167)"><i class="fa fa-angle-right"></i> Rotterdam</a></li>
        <li><a onclick="showCity('Utrecht', 52.090833, 5.122222)"><i class="fa fa-angle-right"></i> Utrecht</a></li>
    </ol>
</li>
<li id="country-NZ"><a class="country" onclick="toggleCountry('NZ')"><img src="img/flag/NZ.png" alt="New Zealand"/>New Zealand</a>
    <ol id="cities-NZ">
        <li><a onclick="showCity('Auckland', -36.8666667, 174.7666667)"><i class="fa fa-angle-right"></i> Auckland</a></li>
        <li><a onclick="showCity('Christchurch', -43.5333333, 172.6333333)"><i class="fa fa-angle-right"></i> Christchurch</a></li>
        <li><a onclick="showCity('Manukau City', -36.9817120959048, 174.870758056641)"><i class="fa fa-angle-right"></i> Manukau City</a></li>
        <li><a onclick="showCity('Wellington', -41.2785361586459, 174.776639342308)"><i class="fa fa-angle-right"></i> Wellington</a></li>
    </ol>
</li>
<li id="country-NE"><a class="country" onclick="toggleCountry('NE')"><img src="img/flag/NE.png" alt="Niger"/>Niger</a>
    <ol id="cities-NE">
        <li><a onclick="showCity('Niamey', 13.5166667, 2.1166667)"><i class="fa fa-angle-right"></i> Niamey</a></li>
    </ol>
</li>
<li id="country-NG"><a class="country" onclick="toggleCountry('NG')"><img src="img/flag/NG.png" alt="Nigeria"/>Nigeria</a>
    <ol id="cities-NG">
        <li><a onclick="showCity('Aba', 5.1166667, 7.3666667)"><i class="fa fa-angle-right"></i> Aba</a></li>
        <li><a onclick="showCity('Benin-City', 6.3333333, 5.6333333)"><i class="fa fa-angle-right"></i> Benin-City</a></li>
        <li><a onclick="showCity('Ibadan', 7.3877778, 3.8963889)"><i class="fa fa-angle-right"></i> Ibadan</a></li>
        <li><a onclick="showCity('Jos', 9.9166667, 8.9)"><i class="fa fa-angle-right"></i> Jos</a></li>
        <li><a onclick="showCity('Kaduna', 10.5230556, 7.4402778)"><i class="fa fa-angle-right"></i> Kaduna</a></li>
        <li><a onclick="showCity('Kano', 11.9963889, 8.5166667)"><i class="fa fa-angle-right"></i> Kano</a></li>
        <li><a onclick="showCity('Lagos', 6.4530556, 3.3958333)"><i class="fa fa-angle-right"></i> Lagos</a></li>
        <li><a onclick="showCity('Maiduguri', 11.845, 13.16)"><i class="fa fa-angle-right"></i> Maiduguri</a></li>
        <li><a onclick="showCity('Port Harcourt', 4.7891667, 6.9986111)"><i class="fa fa-angle-right"></i> Port Harcourt</a></li>
        <li><a onclick="showCity('Zaria', 11.0666667, 7.7)"><i class="fa fa-angle-right"></i> Zaria</a></li>
    </ol>
</li>
<li id="country-KP"><a class="country" onclick="toggleCountry('KP')"><img src="img/flag/KP.png" alt="North Korea"/>North Korea</a>
    <ol id="cities-KP">
        <li><a onclick="showCity('Chongjin', 41.7955556, 129.7758333)"><i class="fa fa-angle-right"></i> Chongjin</a></li>
        <li><a onclick="showCity('Hamh\u016Dng', 39.9183333, 127.5363889)"><i class="fa fa-angle-right"></i> Hamh&#365;ng</a></li>
        <li><a onclick="showCity('H\u016Dngnam', 39.8316667, 127.6186111)"><i class="fa fa-angle-right"></i> H&#365;ngnam</a></li>
        <li><a onclick="showCity('Kaes\u014Fng', 37.9708333, 126.5544444)"><i class="fa fa-angle-right"></i> Kaes&#335;ng</a></li>
        <li><a onclick="showCity('Namp\u2019o', 38.7375, 125.4077778)"><i class="fa fa-angle-right"></i> Namp&rsquo;o</a></li>
        <li><a onclick="showCity('Pyongyang', 39.0194444, 125.7547222)"><i class="fa fa-angle-right"></i> Pyongyang</a></li>
        <li><a onclick="showCity('Sin\u016Diju', 40.1005556, 124.3980556)"><i class="fa fa-angle-right"></i> Sin&#365;iju</a></li>
        <li><a onclick="showCity('W\u014Fnsan', 39.1527778, 127.4436111)"><i class="fa fa-angle-right"></i> W&#335;nsan</a></li>
    </ol>
</li>
<li id="country-PK"><a class="country" onclick="toggleCountry('PK')"><img src="img/flag/PK.png" alt="Pakistan"/>Pakistan</a>
    <ol id="cities-PK">
        <li><a onclick="showCity('Faisal\u0101b\u0101d', 31.4166667, 73.0833333)"><i class="fa fa-angle-right"></i> Faisal&#257;b&#257;d</a></li>
        <li><a onclick="showCity('Gujr\u0101nw\u0101la', 32.15, 74.1833333)"><i class="fa fa-angle-right"></i> Gujr&#257;nw&#257;la</a></li>
        <li><a onclick="showCity('Hyder\u0101b\u0101d', 25.3666667, 68.3666667)"><i class="fa fa-angle-right"></i> Hyder&#257;b&#257;d</a></li>
        <li><a onclick="showCity('Islamabad', 33.6100445736956, 73.05908203125)"><i class="fa fa-angle-right"></i> Islamabad</a></li>
        <li><a onclick="showCity('Kar\u0101chi', 24.8666667, 67.05)"><i class="fa fa-angle-right"></i> Kar&#257;chi</a></li>
        <li><a onclick="showCity('Lahore', 31.5497222, 74.3436111)"><i class="fa fa-angle-right"></i> Lahore</a></li>
        <li><a onclick="showCity('Mult\u0101n', 30.1955556, 71.4752778)"><i class="fa fa-angle-right"></i> Mult&#257;n</a></li>
        <li><a onclick="showCity('Pesh\u0101war', 34.0077778, 71.5733333)"><i class="fa fa-angle-right"></i> Pesh&#257;war</a></li>
        <li><a onclick="showCity('Quetta', 30.1872222, 67.0125)"><i class="fa fa-angle-right"></i> Quetta</a></li>
        <li><a onclick="showCity('R\u0101walpindi', 33.6, 73.0666667)"><i class="fa fa-angle-right"></i> R&#257;walpindi</a></li>
    </ol>
</li>
<li id="country-PE"><a class="country" onclick="toggleCountry('PE')"><img src="img/flag/PE.png" alt="Peru"/>Peru</a>
    <ol id="cities-PE">
        <li><a onclick="showCity('Arequipa', -16.3988889, -71.535)"><i class="fa fa-angle-right"></i> Arequipa</a></li>
        <li><a onclick="showCity('Callao', -12.0666667, -77.15)"><i class="fa fa-angle-right"></i> Callao</a></li>
        <li><a onclick="showCity('Chiclayo', -6.7736111, -79.8416667)"><i class="fa fa-angle-right"></i> Chiclayo</a></li>
        <li><a onclick="showCity('Chimbote', -9.0852778, -78.5783333)"><i class="fa fa-angle-right"></i> Chimbote</a></li>
        <li><a onclick="showCity('Cusco', -13.5183333, -71.9780556)"><i class="fa fa-angle-right"></i> Cusco</a></li>
        <li><a onclick="showCity('Huancayo', -12.0666667, -75.2333333)"><i class="fa fa-angle-right"></i> Huancayo</a></li>
        <li><a onclick="showCity('Iquitos', -3.7480556, -73.2472222)"><i class="fa fa-angle-right"></i> Iquitos</a></li>
        <li><a onclick="showCity('Lima', -12.05, -77.05)"><i class="fa fa-angle-right"></i> Lima</a></li>
        <li><a onclick="showCity('Piura', -5.2, -80.6333333)"><i class="fa fa-angle-right"></i> Piura</a></li>
        <li><a onclick="showCity('Trujillo', -8.1159942774572, -79.0299797058105)"><i class="fa fa-angle-right"></i> Trujillo</a></li>
    </ol>
</li>
<li id="country-PH"><a class="country" onclick="toggleCountry('PH')"><img src="img/flag/PH.png" alt="Philippines"/>Philippines</a>
    <ol id="cities-PH">
        <li><a onclick="showCity('Antipolo', 14.5863889, 121.1752778)"><i class="fa fa-angle-right"></i> Antipolo</a></li>
        <li><a onclick="showCity('Cagayan de Oro', 8.4822222, 124.6472222)"><i class="fa fa-angle-right"></i> Cagayan de Oro</a></li>
        <li><a onclick="showCity('Cebu City', 10.3111111, 123.8916667)"><i class="fa fa-angle-right"></i> Cebu City</a></li>
        <li><a onclick="showCity('Dasmari\u00F1as', 14.3294444, 120.9366667)"><i class="fa fa-angle-right"></i> Dasmari&ntilde;as</a></li>
        <li><a onclick="showCity('Davao', 7.0730556, 125.6127778)"><i class="fa fa-angle-right"></i> Davao</a></li>
        <li><a onclick="showCity('Iloilo', 10.6969444, 122.5644444)"><i class="fa fa-angle-right"></i> Iloilo</a></li>
        <li><a onclick="showCity('Manila', 14.6041667, 120.9822222)"><i class="fa fa-angle-right"></i> Manila</a></li>
        <li><a onclick="showCity('Mansilingan', 10.6311111, 122.9788889)"><i class="fa fa-angle-right"></i> Mansilingan</a></li>
        <li><a onclick="showCity('San Jose del Monte', 14.8138889, 121.0452778)"><i class="fa fa-angle-right"></i> San Jose del Monte</a></li>
        <li><a onclick="showCity('Zamboanga', 6.9102778, 122.0738889)"><i class="fa fa-angle-right"></i> Zamboanga</a></li>
    </ol>
</li>
<li id="country-PL"><a class="country" onclick="toggleCountry('PL')"><img src="img/flag/PL.png" alt="Poland"/>Poland</a>
    <ol id="cities-PL">
        <li><a onclick="showCity('Bydgoszcz', 53.15, 18.0)"><i class="fa fa-angle-right"></i> Bydgoszcz</a></li>
        <li><a onclick="showCity('Gda\u0144sk', 54.35, 18.6666667)"><i class="fa fa-angle-right"></i> Gda&#324;sk</a></li>
        <li><a onclick="showCity('Katowice', 50.2666667, 19.0166667)"><i class="fa fa-angle-right"></i> Katowice</a></li>
        <li><a onclick="showCity('Krak\u00F3w', 50.0833333, 19.9166667)"><i class="fa fa-angle-right"></i> Krak&oacute;w</a></li>
        <li><a onclick="showCity('Lublin', 51.25, 22.5666667)"><i class="fa fa-angle-right"></i> Lublin</a></li>
        <li><a onclick="showCity('Pozna\u0144', 52.4166667, 16.9666667)"><i class="fa fa-angle-right"></i> Pozna&#324;</a></li>
        <li><a onclick="showCity('Szczecin', 53.4293784371188, 14.5529365539551)"><i class="fa fa-angle-right"></i> Szczecin</a></li>
        <li><a onclick="showCity('Warsaw', 52.25, 21.0)"><i class="fa fa-angle-right"></i> Warsaw</a></li>
        <li><a onclick="showCity('Wroc\u0142aw', 51.1, 17.0333333)"><i class="fa fa-angle-right"></i> Wroc&#322;aw</a></li>
        <li><a onclick="showCity('\u0141\u00F3d\u017A', 51.75, 19.4666667)">&#321;&oacute;d&#378;</a></li>
    </ol>
</li>
<li id="country-PT"><a class="country" onclick="toggleCountry('PT')"><img src="img/flag/PT.png" alt="Portugal"/>Portugal</a>
    <ol id="cities-PT">
        <li><a onclick="showCity('Lisbon', 38.7166667, -9.1333333)"><i class="fa fa-angle-right"></i> Lisbon</a></li>
    </ol>
</li>
<li id="country-RO"><a class="country" onclick="toggleCountry('RO')"><img src="img/flag/RO.png" alt="Romania"/>Romania</a>
    <ol id="cities-RO">
        <li><a onclick="showCity('Brass\u00F3', 45.6486083838803, 25.6061267852783)"><i class="fa fa-angle-right"></i> Brass&oacute;</a></li>
        <li><a onclick="showCity('Bucure\u015Fti', 44.4333333, 26.1)"><i class="fa fa-angle-right"></i> Bucure&#351;ti</a></li>
        <li><a onclick="showCity('Cluj-Napoca / Kolozsv\u00E1r', 46.7666667, 23.6)"><i class="fa fa-angle-right"></i> Cluj-Napoca / Kolozsv&aacute;r</a></li>
        <li><a onclick="showCity('Constan\u0163a', 44.1833333, 28.65)"><i class="fa fa-angle-right"></i> Constan&#355;a</a></li>
        <li><a onclick="showCity('Craiova', 44.3166667, 23.8)"><i class="fa fa-angle-right"></i> Craiova</a></li>
        <li><a onclick="showCity('Gala\u0163i', 45.45, 28.05)"><i class="fa fa-angle-right"></i> Gala&#355;i</a></li>
        <li><a onclick="showCity('J\u00E1szv\u00E1s\u00E1r', 47.1666667, 27.6)"><i class="fa fa-angle-right"></i> J&aacute;szv&aacute;s&aacute;r</a></li>
        <li><a onclick="showCity('Timi\u015Foara / Temesv\u00E1r', 45.7494444, 21.2272222)"><i class="fa fa-angle-right"></i> Timi&#351;oara / Temesv&aacute;r</a></li>
    </ol>
</li>
<li id="country-RU"><a class="country" onclick="toggleCountry('RU')"><img src="img/flag/RU.png" alt="Russia"/>Russia</a>
    <ol id="cities-RU">
        <li><a onclick="showCity('Chelyabinsk', 55.1544444, 61.4297222)"><i class="fa fa-angle-right"></i> Chelyabinsk</a></li>
        <li><a onclick="showCity('Kazan\'', 55.75, 49.1333333)"><i class="fa fa-angle-right"></i> Kazan'</a></li>
        <li><a onclick="showCity('Moscow', 55.7522222, 37.6155556)"><i class="fa fa-angle-right"></i> Moscow</a></li>
        <li><a onclick="showCity('Nizhniy Novgorod', 56.3286733202717, 44.002046585083)"><i class="fa fa-angle-right"></i> Nizhniy Novgorod</a></li>
        <li><a onclick="showCity('Novosibirsk', 55.0411111, 82.9344444)"><i class="fa fa-angle-right"></i> Novosibirsk</a></li>
        <li><a onclick="showCity('Omsk', 55.0, 73.4)"><i class="fa fa-angle-right"></i> Omsk</a></li>
        <li><a onclick="showCity('Rostov-na-Donu', 47.2363889, 39.7138889)"><i class="fa fa-angle-right"></i> Rostov-na-Donu</a></li>
        <li><a onclick="showCity('Saint Petersburg', 59.8944444, 30.2641667)"><i class="fa fa-angle-right"></i> Saint Petersburg</a></li>
        <li><a onclick="showCity('Samara', 53.2, 50.15)"><i class="fa fa-angle-right"></i> Samara</a></li>
        <li><a onclick="showCity('Yekaterinburg', 56.8575, 60.6125)"><i class="fa fa-angle-right"></i> Yekaterinburg</a></li>
    </ol>
</li>
<li id="country-RW"><a class="country" onclick="toggleCountry('RW')"><img src="img/flag/RW.png" alt="Rwanda"/>Rwanda</a>
    <ol id="cities-RW">
        <li><a onclick="showCity('Kigali', -1.9536111, 30.0605556)"><i class="fa fa-angle-right"></i> Kigali</a></li>
    </ol>
</li>
<li id="country-SA"><a class="country" onclick="toggleCountry('SA')"><img src="img/flag/SA.png" alt="Saudi Arabia"/>Saudi Arabia</a>
    <ol id="cities-SA">
        <li><a onclick="showCity('Ad Damm\u0101m', 26.4258333, 50.1141667)"><i class="fa fa-angle-right"></i> Ad Damm&#257;m</a></li>
        <li><a onclick="showCity('A\u0163 \u0162\u0101\'if', 21.2702778, 40.4158333)"><i class="fa fa-angle-right"></i> A&#355; &#354;&#257;'if</a></li>
        <li><a onclick="showCity('Buraydah', 26.3316667, 43.9716667)"><i class="fa fa-angle-right"></i> Buraydah</a></li>
        <li><a onclick="showCity('Jiddah', 21.5169444, 39.2191667)"><i class="fa fa-angle-right"></i> Jiddah</a></li>
        <li><a onclick="showCity('Kham\u012Bs Mushayt', 18.3063889, 42.7291667)"><i class="fa fa-angle-right"></i> Kham&#299;s Mushayt</a></li>
        <li><a onclick="showCity('Mecca', 21.4266667, 39.8261111)"><i class="fa fa-angle-right"></i> Mecca</a></li>
        <li><a onclick="showCity('Medina', 24.4686111, 39.6141667)"><i class="fa fa-angle-right"></i> Medina</a></li>
        <li><a onclick="showCity('Riyadh', 24.6408333, 46.7727778)"><i class="fa fa-angle-right"></i> Riyadh</a></li>
        <li><a onclick="showCity('Sul\u0163\u0101nah', 24.5, 39.5833333)"><i class="fa fa-angle-right"></i> Sul&#355;&#257;nah</a></li>
        <li><a onclick="showCity('Tab\u016Bk', 28.3833333, 36.5833333)"><i class="fa fa-angle-right"></i> Tab&#363;k</a></li>
    </ol>
</li>
<li id="country-SN"><a class="country" onclick="toggleCountry('SN')"><img src="img/flag/SN.png" alt="Senegal"/>Senegal</a>
    <ol id="cities-SN">
        <li><a onclick="showCity('Dakar', 14.6951118854684, -17.4438858032227)"><i class="fa fa-angle-right"></i> Dakar</a></li>
        <li><a onclick="showCity('Grand Dakar', 14.7088889, -17.4552778)"><i class="fa fa-angle-right"></i> Grand Dakar</a></li>
        <li><a onclick="showCity('Pikine', 14.75, -17.4)"><i class="fa fa-angle-right"></i> Pikine</a></li>
        <li><a onclick="showCity('Thi\u00E8s Nones', 14.7833333, -16.9666667)"><i class="fa fa-angle-right"></i> Thi&egrave;s Nones</a></li>
    </ol>
</li>
<li id="country-RS"><a class="country" onclick="toggleCountry('RS')"><img src="img/flag/RS.png" alt="Serbia"/>Serbia</a>
    <ol id="cities-RS">
        <li><a onclick="showCity('Belgrade', 44.8186111, 20.4680556)"><i class="fa fa-angle-right"></i> Belgrade</a></li>
        <li><a onclick="showCity('Pristina', 42.6666667, 21.1666667)"><i class="fa fa-angle-right"></i> Pristina</a></li>
    </ol>
</li>
<li id="country-ZA"><a class="country" onclick="toggleCountry('ZA')"><img src="img/flag/ZA.png" alt="South Africa"/>South Africa</a>
    <ol id="cities-ZA">
        <li><a onclick="showCity('Benoni', -26.1833333, 28.3166667)"><i class="fa fa-angle-right"></i> Benoni</a></li>
        <li><a onclick="showCity('Cape Town', -33.9166667, 18.4166667)"><i class="fa fa-angle-right"></i> Cape Town</a></li>
        <li><a onclick="showCity('Durban', -29.85, 31.0166667)"><i class="fa fa-angle-right"></i> Durban</a></li>
        <li><a onclick="showCity('Johannesburg', -26.2, 28.0833333)"><i class="fa fa-angle-right"></i> Johannesburg</a></li>
        <li><a onclick="showCity('Pietermaritzburg', -29.6166667, 30.3833333)"><i class="fa fa-angle-right"></i> Pietermaritzburg</a></li>
        <li><a onclick="showCity('Port Elizabeth', -33.9666667, 25.5833333)"><i class="fa fa-angle-right"></i> Port Elizabeth</a></li>
        <li><a onclick="showCity('Pretoria', -25.7069444, 28.2294444)"><i class="fa fa-angle-right"></i> Pretoria</a></li>
        <li><a onclick="showCity('Soweto', -26.2666667, 27.8666667)"><i class="fa fa-angle-right"></i> Soweto</a></li>
        <li><a onclick="showCity('Tembisa', -25.9988889, 28.2269444)"><i class="fa fa-angle-right"></i> Tembisa</a></li>
        <li><a onclick="showCity('Vereeniging', -26.6666667, 27.9333333)"><i class="fa fa-angle-right"></i> Vereeniging</a></li>
    </ol>
</li>
<li id="country-KR"><a class="country" onclick="toggleCountry('KR')"><img src="img/flag/KR.png" alt="South Korea"/>South Korea</a>
    <ol id="cities-KR">
        <li><a onclick="showCity('Inch\'\u014Fn', 37.4536111, 126.7316667)"><i class="fa fa-angle-right"></i> Inch'&#335;n</a></li>
        <li><a onclick="showCity('Kwangju', 35.1547222, 126.9155556)"><i class="fa fa-angle-right"></i> Kwangju</a></li>
        <li><a onclick="showCity('Puch\'\u014Fn', 37.4988889, 126.7830556)"><i class="fa fa-angle-right"></i> Puch'&#335;n</a></li>
        <li><a onclick="showCity('Pusan', 35.1027778, 129.0402778)"><i class="fa fa-angle-right"></i> Pusan</a></li>
        <li><a onclick="showCity('Seoul', 37.5663889, 126.9997222)"><i class="fa fa-angle-right"></i> Seoul</a></li>
        <li><a onclick="showCity('S\u014Fngnam', 37.4386111, 127.1377778)"><i class="fa fa-angle-right"></i> S&#335;ngnam</a></li>
        <li><a onclick="showCity('Suw\u014Fn', 37.2841667, 127.0191667)"><i class="fa fa-angle-right"></i> Suw&#335;n</a></li>
        <li><a onclick="showCity('Taegu', 35.8702778, 128.5911111)"><i class="fa fa-angle-right"></i> Taegu</a></li>
        <li><a onclick="showCity('Taej\u014Fn', 36.3213889, 127.4197222)"><i class="fa fa-angle-right"></i> Taej&#335;n</a></li>
        <li><a onclick="showCity('Ulsan', 35.5372222, 129.3166667)"><i class="fa fa-angle-right"></i> Ulsan</a></li>
    </ol>
</li>
<li id="country-ES"><a class="country" onclick="toggleCountry('ES')"><img src="img/flag/ES.png" alt="Spain"/>Spain</a>
    <ol id="cities-ES">
        <li><a onclick="showCity('Barcelona', 41.3887868890716, 2.15898513793945)"><i class="fa fa-angle-right"></i> Barcelona</a></li>
        <li><a onclick="showCity('Bilbao', 43.2627062282001, -2.9252815246582)"><i class="fa fa-angle-right"></i> Bilbao</a></li>
        <li><a onclick="showCity('Las Palmas de Gran Canaria', 28.1, -15.4166667)"><i class="fa fa-angle-right"></i> Las Palmas de Gran Canaria</a></li>
        <li><a onclick="showCity('Madrid', 40.4165020941502, -3.70256423950195)"><i class="fa fa-angle-right"></i> Madrid</a></li>
        <li><a onclick="showCity('M\u00E1laga', 36.7201559059499, -4.42034482955933)"><i class="fa fa-angle-right"></i> M&aacute;laga</a></li>
        <li><a onclick="showCity('Murcia', 37.9833333, -1.1166667)"><i class="fa fa-angle-right"></i> Murcia</a></li>
        <li><a onclick="showCity('Palma', 39.5693907933761, 2.65023708343506)"><i class="fa fa-angle-right"></i> Palma</a></li>
        <li><a onclick="showCity('Sevilla', 37.3772222, -5.9869444)"><i class="fa fa-angle-right"></i> Sevilla</a></li>
        <li><a onclick="showCity('Valencia', 39.4697524227712, -0.377386808395386)"><i class="fa fa-angle-right"></i> Valencia</a></li>
        <li><a onclick="showCity('Zaragoza', 41.6560643302881, -0.877339839935303)"><i class="fa fa-angle-right"></i> Zaragoza</a></li>
    </ol>
</li>
<li id="country-LK"><a class="country" onclick="toggleCountry('LK')"><img src="img/flag/LK.png" alt="Sri Lanka"/>Sri Lanka</a>
    <ol id="cities-LK">
        <li><a onclick="showCity('Colombo', 6.9319444, 79.8477778)"><i class="fa fa-angle-right"></i> Colombo</a></li>
    </ol>
</li>
<li id="country-SD"><a class="country" onclick="toggleCountry('SD')"><img src="img/flag/SD.png" alt="Sudan"/>Sudan</a>
    <ol id="cities-SD">
        <li><a onclick="showCity('Al F\u0101shir', 13.6333333, 25.35)"><i class="fa fa-angle-right"></i> Al F&#257;shir</a></li>
        <li><a onclick="showCity('Al Ubayyi\u1E11', 13.1833333, 30.2166667)"><i class="fa fa-angle-right"></i> Al Ubayyi&#7697;</a></li>
        <li><a onclick="showCity('Kassala', 15.4558333, 36.3988889)"><i class="fa fa-angle-right"></i> Kassala</a></li>
        <li><a onclick="showCity('Khartoum', 15.5880556, 32.5341667)"><i class="fa fa-angle-right"></i> Khartoum</a></li>
        <li><a onclick="showCity('K\u016Bst\u012B', 13.1666667, 32.6666667)"><i class="fa fa-angle-right"></i> K&#363;st&#299;</a></li>
        <li><a onclick="showCity('Omdurman', 15.6361111, 32.4372222)"><i class="fa fa-angle-right"></i> Omdurman</a></li>
        <li><a onclick="showCity('Port Sudan', 19.6158333, 37.2163889)"><i class="fa fa-angle-right"></i> Port Sudan</a></li>
        <li><a onclick="showCity('Wad Madan\u012B', 14.4, 33.5333333)"><i class="fa fa-angle-right"></i> Wad Madan&#299;</a></li>
    </ol>
</li>
<li id="country-SY"><a class="country" onclick="toggleCountry('SY')"><img src="img/flag/SY.png" alt="Syria"/>Syria</a>
    <ol id="cities-SY">
        <li><a onclick="showCity('Aleppo', 36.2027778, 37.1586111)"><i class="fa fa-angle-right"></i> Aleppo</a></li>
        <li><a onclick="showCity('Damascus', 33.5, 36.3)"><i class="fa fa-angle-right"></i> Damascus</a></li>
        <li><a onclick="showCity('\u1E28am\u0101h', 35.1333333, 36.75)">&#7720;am&#257;h</a></li>
        <li><a onclick="showCity('\u1E28im\u015F', 34.7333333, 36.7166667)">&#7720;im&#351;</a></li>
        <li><a onclick="showCity('Latakia', 35.5166667, 35.7833333)"><i class="fa fa-angle-right"></i> Latakia</a></li>
    </ol>
</li>
<li id="country-TW"><a class="country" onclick="toggleCountry('TW')"><img src="img/flag/TW.png" alt="Taiwan"/>Taiwan</a>
    <ol id="cities-TW">
        <li><a onclick="showCity('Chi-lung', 25.1313889, 121.7375)"><i class="fa fa-angle-right"></i> Chi-lung</a></li>
        <li><a onclick="showCity('Hualian', 23.9769444, 121.6044444)"><i class="fa fa-angle-right"></i> Hualian</a></li>
        <li><a onclick="showCity('Kao-hsiung', 22.6177778, 120.3013889)"><i class="fa fa-angle-right"></i> Kao-hsiung</a></li>
        <li><a onclick="showCity('T\'ai-chung-shih', 24.1483333, 120.6866667)"><i class="fa fa-angle-right"></i> T'ai-chung-shih</a></li>
        <li><a onclick="showCity('Tainan City', 23.0247638077913, 120.168113708496)"><i class="fa fa-angle-right"></i> Tainan City</a></li>
        <li><a onclick="showCity('Taipei', 25.0391667, 121.525)"><i class="fa fa-angle-right"></i> Taipei</a></li>
    </ol>
</li>
<li id="country-TH"><a class="country" onclick="toggleCountry('TH')"><img src="img/flag/TH.png" alt="Thailand"/>Thailand</a>
    <ol id="cities-TH">
        <li><a onclick="showCity('Bangkok', 13.75, 100.5166667)"><i class="fa fa-angle-right"></i> Bangkok</a></li>
        <li><a onclick="showCity('Samut Prakan', 13.5993363664207, 100.596753358841)"><i class="fa fa-angle-right"></i> Samut Prakan</a></li>
    </ol>
</li>
<li id="country-TN"><a class="country" onclick="toggleCountry('TN')"><img src="img/flag/TN.png" alt="Tunisia"/>Tunisia</a>
    <ol id="cities-TN">
        <li><a onclick="showCity('Sfax', 34.7405556, 10.7602778)"><i class="fa fa-angle-right"></i> Sfax</a></li>
        <li><a onclick="showCity('Tunis', 36.8027778, 10.1797222)"><i class="fa fa-angle-right"></i> Tunis</a></li>
    </ol>
</li>
<li id="country-TR"><a class="country" onclick="toggleCountry('TR')"><img src="img/flag/TR.png" alt="Turkey"/>Turkey</a>
    <ol id="cities-TR">
        <li><a onclick="showCity('Adana', 37.0016667, 35.3288889)"><i class="fa fa-angle-right"></i> Adana</a></li>
        <li><a onclick="showCity('Ankara', 39.9272222, 32.8644444)"><i class="fa fa-angle-right"></i> Ankara</a></li>
        <li><a onclick="showCity('Antalya', 36.9125, 30.6897222)"><i class="fa fa-angle-right"></i> Antalya</a></li>
        <li><a onclick="showCity('Bursa', 40.1916667, 29.0611111)"><i class="fa fa-angle-right"></i> Bursa</a></li>
        <li><a onclick="showCity('\u00C7ankaya', 39.9178995306752, 32.8626823425293)">&Ccedil;ankaya</a></li>
        <li><a onclick="showCity('Diyarbak\u0131r', 37.9188889, 40.2105556)"><i class="fa fa-angle-right"></i> Diyarbak&#305;r</a></li>
        <li><a onclick="showCity('Gaziantep', 37.0594444, 37.3825)"><i class="fa fa-angle-right"></i> Gaziantep</a></li>
        <li><a onclick="showCity('\u0130stanbul', 41.0138429552247, 28.9496612548828)">&#304;stanbul</a></li>
        <li><a onclick="showCity('\u0130zmir', 38.4072222, 27.1502778)">&#304;zmir</a></li>
        <li><a onclick="showCity('Konya', 37.8655556, 32.4825)"><i class="fa fa-angle-right"></i> Konya</a></li>
    </ol>
</li>
<li id="country-UG"><a class="country" onclick="toggleCountry('UG')"><img src="img/flag/UG.png" alt="Uganda"/>Uganda</a>
    <ol id="cities-UG">
        <li><a onclick="showCity('Kampala', 0.3155556, 32.5655556)"><i class="fa fa-angle-right"></i> Kampala</a></li>
    </ol>
</li>
<li id="country-UA"><a class="country" onclick="toggleCountry('UA')"><img src="img/flag/UA.png" alt="Ukraine"/>Ukraine</a>
    <ol id="cities-UA">
        <li><a onclick="showCity('Dnipropetrovsk', 48.45, 34.9833333)"><i class="fa fa-angle-right"></i> Dnipropetrovsk</a></li>
        <li><a onclick="showCity('Donets\'k', 48.0, 37.8)"><i class="fa fa-angle-right"></i> Donets'k</a></li>
        <li><a onclick="showCity('Kharkiv', 50.0, 36.25)"><i class="fa fa-angle-right"></i> Kharkiv</a></li>
        <li><a onclick="showCity('Kiev', 50.4333333, 30.5166667)"><i class="fa fa-angle-right"></i> Kiev</a></li>
        <li><a onclick="showCity('Kryvyy Rih', 47.9166667, 33.35)"><i class="fa fa-angle-right"></i> Kryvyy Rih</a></li>
        <li><a onclick="showCity('L\'viv', 49.8382592353251, 24.0232372283936)"><i class="fa fa-angle-right"></i> L'viv</a></li>
        <li><a onclick="showCity('Mariupol\'', 47.0666667, 37.5)"><i class="fa fa-angle-right"></i> Mariupol'</a></li>
        <li><a onclick="showCity('Mykolayiv', 46.9666667, 32.0)"><i class="fa fa-angle-right"></i> Mykolayiv</a></li>
        <li><a onclick="showCity('Odesa', 46.4774726081453, 30.7326221466064)"><i class="fa fa-angle-right"></i> Odesa</a></li>
        <li><a onclick="showCity('Zaporizhzhya', 47.8166667, 35.1833333)"><i class="fa fa-angle-right"></i> Zaporizhzhya</a></li>
    </ol>
</li>
<li id="country-GB"><a class="country" onclick="toggleCountry('GB')"><img src="img/flag/GB.png" alt="United Kingdom"/>United Kingdom</a>
    <ol id="cities-GB">
        <li><a onclick="showCity('Birmingham', 52.4666667, -1.9166667)"><i class="fa fa-angle-right"></i> Birmingham</a></li>
        <li><a onclick="showCity('Bristol', 51.45, -2.5833333)"><i class="fa fa-angle-right"></i> Bristol</a></li>
        <li><a onclick="showCity('Edinburgh', 55.95, -3.2)"><i class="fa fa-angle-right"></i> Edinburgh</a></li>
        <li><a onclick="showCity('Glasgow', 55.8651497187961, -4.25763130187988)"><i class="fa fa-angle-right"></i> Glasgow</a></li>
        <li><a onclick="showCity('Leeds', 53.7964812592701, -1.54784917831421)"><i class="fa fa-angle-right"></i> Leeds</a></li>
        <li><a onclick="showCity('Leicester', 52.6333333, -1.1333333)"><i class="fa fa-angle-right"></i> Leicester</a></li>
        <li><a onclick="showCity('Liverpool', 53.410580680167, -2.97793865203857)"><i class="fa fa-angle-right"></i> Liverpool</a></li>
        <li><a onclick="showCity('London', 51.5084152563931, -0.125532746315002)"><i class="fa fa-angle-right"></i> London</a></li>
        <li><a onclick="showCity('Manchester', 53.4809464283615, -2.23743438720703)"><i class="fa fa-angle-right"></i> Manchester</a></li>
        <li><a onclick="showCity('Sheffield', 53.3829700049879, -1.46590232849121)"><i class="fa fa-angle-right"></i> Sheffield</a></li>
    </ol>
</li>
<li id="country-US"><a class="country" onclick="toggleCountry('US')"><img src="img/flag/US.png" alt="United States"/>United States</a>
    <ol id="cities-US">
        <li><a onclick="showCity('Brooklyn', 40.6501038, -73.9495823)"><i class="fa fa-angle-right"></i> Brooklyn</a></li>
        <li><a onclick="showCity('Chicago', 41.850033, -87.6500523)"><i class="fa fa-angle-right"></i> Chicago</a></li>
        <li><a onclick="showCity('Dallas', 32.7830556, -96.8066667)"><i class="fa fa-angle-right"></i> Dallas</a></li>
        <li><a onclick="showCity('Houston', 29.7632836, -95.3632715)"><i class="fa fa-angle-right"></i> Houston</a></li>
        <li><a onclick="showCity('Los Angeles', 34.0522342, -118.2436849)"><i class="fa fa-angle-right"></i> Los Angeles</a></li>
        <li><a onclick="showCity('New York City', 40.7142691, -74.0059729)"><i class="fa fa-angle-right"></i> New York City</a></li>
        <li><a onclick="showCity('Philadelphia', 39.952335, -75.163789)"><i class="fa fa-angle-right"></i> Philadelphia</a></li>
        <li><a onclick="showCity('Phoenix', 33.4483771, -112.0740373)"><i class="fa fa-angle-right"></i> Phoenix</a></li>
        <li><a onclick="showCity('San Antonio', 29.4241219, -98.4936282)"><i class="fa fa-angle-right"></i> San Antonio</a></li>
        <li><a onclick="showCity('San Diego', 32.7153292, -117.1572551)"><i class="fa fa-angle-right"></i> San Diego</a></li>
    </ol>
</li>
<li id="country-UZ"><a class="country" onclick="toggleCountry('UZ')"><img src="img/flag/UZ.png" alt="Uzbekistan"/>Uzbekistan</a>
    <ol id="cities-UZ">
        <li><a onclick="showCity('Andijon', 40.7830556, 72.3438889)"><i class="fa fa-angle-right"></i> Andijon</a></li>
        <li><a onclick="showCity('Namangan', 40.9982978489732, 71.6725730895996)"><i class="fa fa-angle-right"></i> Namangan</a></li>
        <li><a onclick="showCity('Samarqand', 39.6541667, 66.9597222)"><i class="fa fa-angle-right"></i> Samarqand</a></li>
        <li><a onclick="showCity('Tashkent', 41.3166667, 69.25)"><i class="fa fa-angle-right"></i> Tashkent</a></li>
    </ol>
</li>
<li id="country-VE"><a class="country" onclick="toggleCountry('VE')"><img src="img/flag/VE.png" alt="Venezuela"/>Venezuela</a>
    <ol id="cities-VE">
        <li><a onclick="showCity('Barcelona', 10.1333333, -64.7)"><i class="fa fa-angle-right"></i> Barcelona</a></li>
        <li><a onclick="showCity('Barquisimeto', 10.0738889, -69.3227778)"><i class="fa fa-angle-right"></i> Barquisimeto</a></li>
        <li><a onclick="showCity('Caracas', 10.5, -66.9166667)"><i class="fa fa-angle-right"></i> Caracas</a></li>
        <li><a onclick="showCity('Ciudad Guayana', 8.3533333, -62.6527778)"><i class="fa fa-angle-right"></i> Ciudad Guayana</a></li>
        <li><a onclick="showCity('Maracaibo', 10.6316667, -71.6405556)"><i class="fa fa-angle-right"></i> Maracaibo</a></li>
        <li><a onclick="showCity('Matur\u00EDn', 9.75, -63.1766667)"><i class="fa fa-angle-right"></i> Matur&iacute;n</a></li>
        <li><a onclick="showCity('Petare', 10.4833333, -66.8166667)"><i class="fa fa-angle-right"></i> Petare</a></li>
        <li><a onclick="showCity('Puerto La Cruz', 10.2166667, -64.6166667)"><i class="fa fa-angle-right"></i> Puerto La Cruz</a></li>
        <li><a onclick="showCity('Turmero', 10.2333333, -67.4833333)"><i class="fa fa-angle-right"></i> Turmero</a></li>
        <li><a onclick="showCity('Valencia', 10.1805556, -68.0038889)"><i class="fa fa-angle-right"></i> Valencia</a></li>
    </ol>
</li>
<li id="country-VN"><a class="country" onclick="toggleCountry('VN')"><img src="img/flag/VN.png" alt="Vietnam"/>Vietnam</a>
    <ol id="cities-VN">
        <li><a onclick="showCity('Bi\u00EAn H\u00F2a', 10.95, 106.8166667)"><i class="fa fa-angle-right"></i> Bi&ecirc;n H&ograve;a</a></li>
        <li><a onclick="showCity('C\u1EA7n Th\u01A1', 10.0333333, 105.7833333)"><i class="fa fa-angle-right"></i> C&#7847;n Th&#417;</a></li>
        <li><a onclick="showCity('Da Nang', 16.0677778, 108.2208333)"><i class="fa fa-angle-right"></i> Da Nang</a></li>
        <li><a onclick="showCity('H\u00E0 N\u1ED9i', 21.0333333, 105.85)"><i class="fa fa-angle-right"></i> H&agrave; N&#7897;i</a></li>
        <li><a onclick="showCity('Haiphong', 20.8561111, 106.6822222)"><i class="fa fa-angle-right"></i> Haiphong</a></li>
        <li><a onclick="showCity('Hu\u1EBF', 16.4666667, 107.6)"><i class="fa fa-angle-right"></i> Hu&#7871;</a></li>
        <li><a onclick="showCity('Nha Trang', 12.25, 109.1833333)"><i class="fa fa-angle-right"></i> Nha Trang</a></li>
        <li><a onclick="showCity('Th\u00E0nh ph\u1ED1 H\u1ED3 Ch\u00ED Minh', 10.75, 106.6666667)"><i class="fa fa-angle-right"></i> Th&agrave;nh ph&#7889; H&#7891; Ch&iacute; Minh</a></li>
    </ol>
</li>
<li id="country-ZM"><a class="country" onclick="toggleCountry('ZM')"><img src="img/flag/ZM.png" alt="Zambia"/>Zambia</a>
    <ol id="cities-ZM">
        <li><a onclick="showCity('Kitwe', -12.8166667, 28.2)"><i class="fa fa-angle-right"></i> Kitwe</a></li>
        <li><a onclick="showCity('Lusaka', -15.4166667, 28.2833333)"><i class="fa fa-angle-right"></i> Lusaka</a></li>
        <li><a onclick="showCity('Ndola', -12.9666667, 28.6333333)"><i class="fa fa-angle-right"></i> Ndola</a></li>
    </ol>
</li>
<li id="country-ZW"><a class="country" onclick="toggleCountry('ZW')"><img src="img/flag/ZW.png" alt="Zimbabwe"/>Zimbabwe</a>
    <ol id="cities-ZW">
        <li><a onclick="showCity('Bulawayo', -20.15, 28.5833333)"><i class="fa fa-angle-right"></i> Bulawayo</a></li>
        <li><a onclick="showCity('Chitungwiza', -17.9938889, 31.0480556)"><i class="fa fa-angle-right"></i> Chitungwiza</a></li>
        <li><a onclick="showCity('Harare', -17.8177778, 31.0447222)"><i class="fa fa-angle-right"></i> Harare</a></li>
    </ol>
</li>
